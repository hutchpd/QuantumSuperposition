...
\texttt{DOI: 10.5281/zenodo.17863969}
}
...
\begin{center}
\small
Technical report, Findon Software. Current version: December 2025.\\
DOI: \texttt{10.5281/zenodo.17863969}
\end{center}
...
This paper describes the aligned \textbf{v1.9} release line of both libraries.
...
\footnote{In the generic layer, Select acts as an \textbf{incoherent push-forward} of the
eigenstate probability distribution: branch probabilities are transported through the mapping,
and when targets collide their \textbf{probability masses} are added. Coherent amplitude
addition, phase-sensitive cancellation, and other interference effects are reserved for explicit
quantum operations in the physics layer (See Tables~\ref{tab:select-double}--\ref{tab:select-merge}).}
...
However, the call to Select(x =\textgreater{} x * 2) does not collapse the state. Instead, in the generic layer it pushes forward the eigenstate \textbf{probability distribution} through the pure function x ↦ 2x, giving us a new superposition in which:
...
\item
  because this map is injective on \{1,2,3\}, each branch keeps its original probability mass.
\end{enumerate}
\begin{table}[htbp]
  \centering
  \begin{tabular}{@{}
...
In this toy example the function x ↦ 2x is injective on \{1, 2, 3\}, so no two branches of the multiverse crash into the same target value. If we instead used a function that is not injective, such as x ↦ x \% 2, then multiple eigenstates would map to the same result and their probability masses would aggregate.
...
For generic Select, this is a straight probability push-forward: start from P(1)=P(2)=P(3)=1/3, then add source masses that land on the same target. So P(even)=P(2)=1/3 and P(odd)=P(1)+P(3)=1/3+1/3=2/3. Table~\ref{tab:select-merge} shows the same aggregation in tabular form:
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}
    ...
    >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}@{}}
  	\toprule
  	\textbf{Eigenstate (value)} & \textbf{Contributing sources} & \textbf{Source probability contributions} & \textbf{Aggregated probability mass} \\
  \midrule
  0 (even) & 2 & 1/3 & 1/3 \\
  1 (odd) & 1, 3 & 1/3 + 1/3 & 2/3 \\
  \bottomrule
  \end{tabular}
  \caption{Probability-mass aggregation after generic Select maps multiple branches to the same value}
...
QuBit\textless T\textgreater{} stores eigenstates with weights from which probabilities are derived.

Generic Select pushes forward branch probabilities through the provided function and adds probability mass when keys collide (an incoherent many-to-one map, not a unitary transform).
...
\end{enumerate}
 ...
 \paragraph{Sketch and forward reference.}\mbox{}\\
